...
def generate_prompt(CQs, procedure="", combined_patterns="", previous_output=""):
    return (
        f"Read the following instructions: '{procedure}'. Basing on the procedure, and following the previous output: "
...
def design_ontology(prompt):
    ...
    logging.info(
        f"Response at {datetime.now()}: {response.choices[0].message.content.strip()}"
    )
...
def main(CQs, procedure=None, combined_patterns=None, iteration=1):
    ...
    for group_number, CQs_group in enumerate(CQs, start=1):
        prompt = generate_prompt(
            ...
            combined_patterns if combined_patterns else "",
            previous_output,
        )
        ...
        previous_output += f"\n\n{ontology_output}"
        logging.info(
            f"Group {group_number} processed. Tokens used: {tokens_used}. Total tokens used: {total_tokens_used}"
        )
    ...
    output_file_name = (
        f"output_{case_str}_{date_str}_trial{iteration}-gpt-4-0125-preview-trial3.owl"
    )
...
if __name__ == "__main__":
    ...
    combined_pattern_str = ". ".join(
        [f"{row['Name']}: {row['Pattern_owl']}" for _, row in pattern_data.iterrows()]
    )
    ...
    main(
        ...
        combined_patterns=combined_pattern_str,
        iteration=iteration,
    )
